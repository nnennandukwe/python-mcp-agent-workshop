...
<task type="auto">
  ...
  <action>
    ...
    4. **KeyError sanitization** - Verify key name not exposed
       - Trigger KeyError for missing required argument
...
<task type="auto">
  ...
  <action>
    ...
    **Leak points 4 & 5 (lines ~416, ~523 - ValueError/FileNotFoundError/SyntaxError):**
    ...
    **Wrap _serve_once in request_context:**
    ```python
    def _serve_once(self, stdin: Any, stdout: Any) -> bool:
        with request_context() as correlation_id:
            try:
                ...
                # Use correlation_id from context
    ```
  ...
  <verify>poetry run pytest tests/test_error_sanitization.py -v</verify>
  <done>All 6 leak points return generic messages; full details logged with correlation ID</done>
</task>
