...
"""
    ...
    @pytest.mark.asyncio
    async def test_basic_keyword_search(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
        ...
        files_with_world = [
            file_path for file_path, data in result["files"].items()
    ...
    @pytest.mark.asyncio
    async def test_case_sensitivity(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_multiple_root_paths(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_empty_keyword_error(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_file_as_root_path_error(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_no_matches_found(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_file_extension_filtering(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
        """Test that only supported text file extensions are processed."""
        result = await search_tool.execute("world", [str(temp_test_directory)])
        
        # Check that binary files are not included in results
        binary_files = [
            file_path for file_path in result["files"].keys()
    ...
    @pytest.mark.asyncio
    async def test_empty_file_handling(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_nested_directory_search(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    @pytest.mark.asyncio
    async def test_summary_calculations(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
        ...
        calculated_total = sum(
            data["occurrences"] for data in result["files"].values()
        )
        ...
        expected_percentage = (
            ...
            if summary["total_files_searched"] > 0 else 0.0
        )
    ...
    @pytest.mark.asyncio
    async def test_most_frequent_file_identification(self, search_tool: KeywordSearchTool, temp_test_directory: Path):
    ...
    async def test_concurrent_search_performance(self, search_tool: KeywordSearchTool):
        ...
        try:
            ...
            search_tasks = [
                search_tool.execute("world", [str(temp_dir)])
    ...
    def test_is_text_file_method(self, search_tool: KeywordSearchTool):
        ...
        supported_files = [
            Path("test.py"), Path("test.java"), Path("test.js"), Path("test.ts"),
            Path("test.html"), Path("test.css"), Path("test.json"), Path("test.xml"),
            Path("test.md"), Path("test.txt"), Path("test.yml"), Path("test.yaml")
        ]
        ...
        unsupported_files = [
            Path("test.bin"), Path("test.exe"), Path("test.pdf"), Path("test.jpg"),
            Path("test.png"), Path("test.zip"), Path("test.tar.gz")
        ]
        
        for file_path in unsupported_files:
            assert not search_tool._is_text_file(file_path), f"Should not support {file_path.suffix}"